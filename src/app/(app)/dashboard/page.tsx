...
export default async function DashboardPage() {
  ...
  const rows = await Promise.all(
    competitions.map(async (c) => {
      ...
      const md = await prisma.matchday.findFirst({
        where: { competitionId: c.id, number: focus.number },
        select: {
          id: true,
          number: true,
          deadlineAt: true,
      ...
      return { c, md, tipped, total, open: isTippable(md.deadlineAt) };
    }),
  );
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Wochenende-Hero: der wichtigste Tipp-Slot der Woche. */}
      {focusRow && <WeekendHero row={focusRow} />}
      ...
      {/* Tippleitungs-Hinweis für Admins */}
        ...
        <div className="space-y-4">
          ...
          <div className="text-muted-foreground flex flex-wrap items-center gap-x-4 gap-y-1 text-base">
            <span className="flex items-center gap-1.5">
              ...
              Deadline{' '}
              <span className="text-foreground font-medium">{deadlineLabel}</span>
            </span>
            <span aria-hidden="true">·</span>
            <span className="font-mono text-xs">
              {row.md.sections
                .map((s) => (s.league ? LEAGUE_SECTION_LABELS[s.league] : 'Liga'))
                .join(' + ')}
            </span>
          </div>
          ...
          <div className="space-y-2 pt-1">
            <div className="flex items-baseline gap-2">
              <span className="font-display text-3xl font-semibold tabular-nums">
                {row.tipped}
                <span className="text-muted-foreground text-xl">/{row.total}</span>
              </span>
  ...
  return (
    ...
    >
      ...
      <div className="text-muted-foreground flex items-center gap-2 text-xs">
        ...
        <span aria-hidden="true">·</span>
        <span>
          {row.open ? 'offen' : row.tipped === row.total ? 'vollständig' : 'geschlossen'}
        </span>
      </div>
    </Link>
  );
