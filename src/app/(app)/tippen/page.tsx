...
import { getCompetitions, getMatchdayByNumber, isTippable } from '@/lib/matchdays';
...
export default async function TippenPage({
  ...
  const selectedNumber = numbers.includes(requestedNumber)
    ? requestedNumber
    : (competition.matchdays.find((m) => m.isActive)?.number ?? numbers[numbers.length - 1]);
    ...
    <div className="space-y-6">
      ...
      <nav className="flex flex-wrap gap-2">
        {competitions.map((c) => {
          ...
          const target = numberForLink(c.matchdays, sp, c.key === selectedKey ? selectedNumber : undefined);
          return (
            <Link
              key={c.key}
...
/** Spieltag-Nummer, die ein Wettbewerb-Tab anspringen soll (aktiver oder letzter). */
function numberForLink(
  matchdays: { number: number; isActive: boolean }[],
  sp: { matchday?: string },
  ...
  const active = matchdays.find((m) => m.isActive)?.number;
  return active ?? matchdays[matchdays.length - 1].number;
}
